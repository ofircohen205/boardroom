...
from backend.shared.ai.state.result_state import ComparisonResult, StockRanking
from backend.shared.ai.tools.relative_strength import calculate_relative_strength
...
class BoardroomGraph:
    ...
    async def _generate_comparison(
        ...
        from backend.shared.ai.prompts import format_comparison_prompt

        # Build comparison data for prompt
        comparison_data = []
        for ticker in tickers:
        ...
        # Use refactored prompt template
        from backend.shared.ai.prompts import (
            COMPARISON_SCHEMA,
        )

        prompt = format_comparison_prompt(tickers, comparison_data)

        # Use structured output for reliable JSON parsing
        try:
            parsed = await self.chairperson.llm.complete_structured(
                [{"role": "user", "content": prompt}],
                json_schema=COMPARISON_SCHEMA,
            )

            best_pick = parsed.get("best_pick", tickers[0] if tickers else "")
                ...
                if ticker and ticker in all_results:
                    ...
                    if decision:
                        rankings.append(
                            StockRanking(
                                ticker=ticker,
                                rank=ranking_data.get("rank", len(rankings) + 1),
                                score=ranking_data.get(
                                    "score", int(decision["confidence"] * 100)
                                ),
                                rationale=ranking_data.get(
            ...
            # Fallback: use default ranking based on confidence scores
            best_pick = tickers[0] if tickers else ""
            summary = f"Comparison analysis completed (fallback mode due to: {type(e).__name__})"
            rankings = []
