    ...
    return [
        {
            "id": str(w.id),
            "name": w.name,
            "items": [{"id": str(i.id), "ticker": i.ticker, "market": i.market.value} for i in w.items]
        }
        for w in watchlists
    ]
...
async def create_new_watchlist(
    ...
    return {"id": str(watchlist.id), "name": watchlist.name, "items": []}
...
async def add_item(
    ...
    return {"id": str(item.id), "ticker": item.ticker, "market": item.market.value}
