...
def is_us_market_hours() -> bool:
    ...
    market_open = now.replace(hour=US_MARKET_OPEN_HOUR, minute=US_MARKET_OPEN_MINUTE).time()
    market_close = now.replace(hour=US_MARKET_CLOSE_HOUR, minute=US_MARKET_CLOSE_MINUTE).time()
...
def is_tase_market_hours() -> bool:
    ...
    # Check if within market hours
    current_time = now.time()
    market_open = now.replace(hour=TASE_MARKET_OPEN_HOUR, minute=TASE_MARKET_OPEN_MINUTE).time()
    market_close = now.replace(hour=TASE_MARKET_CLOSE_HOUR, minute=TASE_MARKET_CLOSE_MINUTE).time()
...
def check_alert_condition(alert, current_price: float) -> bool:
    ...
    if alert.condition == AlertCondition.ABOVE:
        ...
        if baseline_price is None or baseline_price == 0:
            logger.warning(f"Alert {alert.id}: No baseline price for change_pct alert on {alert.ticker}, skipping")
            return False

        pct_change = abs((current_price - baseline_price) / baseline_price * 100)
        logger.debug(f"Alert {alert.id}: {alert.ticker} change: {pct_change:.2f}% (current: ${current_price}, baseline: ${baseline_price}, threshold: {alert.target_value}%)")
...
async def check_price_alerts(db: AsyncSession) -> dict:
    ...
    try:
        ...
        logger.info(f"Markets open - US: {us_open}, TASE: {tase_open}. Checking {len(open_tickers)} tickers")
        ...
        for ticker, market in open_tickers:
            try:
                ...
                logger.debug(f"Fetched price for {ticker} ({market.value}): ${prices[(ticker, market)]}")
            except Exception as e:
                logger.error(f"Failed to fetch price for {ticker} ({market.value}): {e}")
            ...
            for alert in alerts:
                ...
                if check_alert_condition(alert, current_price):
                    try:
                        ...
                        alerts_triggered += 1
                        logger.info(f"Triggered alert {alert.id} for {ticker} at ${current_price}")
                    except Exception as e:
        ...
        logger.info(f"Alert checker completed: {alerts_checked} checked, {alerts_triggered} triggered in {duration:.2f}s")
