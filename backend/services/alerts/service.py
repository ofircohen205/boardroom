...
async def create_price_alert(
    ...
    if condition == AlertCondition.CHANGE_PCT:
        if target_value < 0.1 or target_value > 100:
            raise AlertValidationError("Change percentage must be between 0.1 and 100")
...
async def trigger_alert(
    ...
    alert.cooldown_until = datetime.now() + timedelta(minutes=ALERT_COOLDOWN_MINUTES)
    await alert_dao.update(alert)

    # Check for recent notification to group with (within 15 minutes)
    recent_notification = await notification_dao.find_recent_by_ticker(
    ...
    try:
        ...
        logger.error(f"Failed to send WebSocket notification for alert {alert.id}: {e}")
