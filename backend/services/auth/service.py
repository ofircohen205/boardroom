...
# For backward compatibility, create module-level functions that use the service
async def register_user(
...
async def login_user(email: str, password: str, db: AsyncSession) -> tuple[User, str]:
    ...
    """
    service = AuthService(UserDAO(db), WatchlistDAO(db), PortfolioDAO(db))
    return await service.login_user(email, password, db)
...
async def authenticate_user(email: str, db: AsyncSession) -> Optional[User]:
    ...
    """
    service = AuthService(UserDAO(db), WatchlistDAO(db), PortfolioDAO(db))
    return await service.authenticate_user(email)
